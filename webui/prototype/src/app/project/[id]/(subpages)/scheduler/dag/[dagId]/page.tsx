...
/**
 * DAG 详情页
 * 页面路径: /project/[id]/scheduler/dag/[dagId]
 */

import { useState, useCallback, useMemo } from "react";
import { useParams, useRouter } from "next/navigation";
import { Card, Button, Space, message, Select, Input, Collapse, InputNumber, Modal, Tag, Descriptions, Table, Typography } from "antd";
import { Save, Play, CheckCircle, Maximize2, Database, FileSearch, Shield, Code, Terminal, GitBranch, RefreshCw, ArrowLeft, Edit } from "lucide-react";
...
export default function DAGDetailPage() {
  ...
  const dag = mockDAGTasks.find((d) => d.id === dagId) || mockDAGTasks[0];

  const [nodes, setNodes] = useState<DAGNode[]>(dag.nodes);
  const [edges, setEdges] = useState<DAGEdge[]>(dag.edges);
  const [selectedNode, setSelectedNode] = useState<DAGNode | null>(null);
  const [validationResult, setValidationResult] = useState<{valid: boolean; errors: string[]; warnings: string[]} | null>(null);
  const [isFullscreen, setIsFullscreen] = useState(false);

  const handleBack = () => {
    router.push(`/project/${projectId}/scheduler`);
  };

  const breadcrumbItems: BreadcrumbItem[] = useMemo(() => [
    { title: "数据项目", href: ROUTES.PROJECT },
    { title: project?.name || "项目", href: `/project/${projectId}` },
    { title: "调度中心", href: `/project/${projectId}/scheduler` },
    { title: dag.name },
  ], [projectId, project, dag.name]);
  ...
  return (
    <PageLayout title={`${dag.name} - DAG编排`}>
        ...
        <Card style={{ marginBottom: 16 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
              ...
              <Title level={4} style={{ margin: 0 }}>{dag.name}</Title>
              <Tag color={dagStatusColors[dag.status]}>{dagStatusLabels[dag.status]}</Tag>
            </div>
            <Space>
              ...
              <Button icon={<Maximize2 size={14} />} onClick={handleFullscreen} type={isFullscreen ? "primary" : "default"}>
      ...
      <div style={{ display: "flex", gap: 16, height: isFullscreen ? "calc(100vh - 80px)" : "calc(100vh - 320px)", minHeight: 500, ...fullscreenStyle }}>
        {!isFullscreen && (
